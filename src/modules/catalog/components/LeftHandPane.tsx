...
export default function LeftHandPane({ isClientSet, clientData, selectProduct }: { isClientSet: boolean, clientData: any, selectProduct: (id: string, uniqueId: string) => void }) {
                ...
                {isClientSet ?
                    (isLoading ? (
                        ...
                        productsList.map((product: any) => (
                            <div className="flex justify-start items-start hover:cursor-pointer pl-[12px] h-[450px]" key={product.id}>
                                <Products
                                    id={product.id}
                                    uniqueId={product.uniqueId}
                                    className="h-[450px] w-[230px] hover:cursor-pointer"
                                    title={product.title}
